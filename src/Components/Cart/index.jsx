			...
			<Wrapper>
						...
						state?.cart?.map(value => {
							return (
								<Product key={value?.id}>
									...
									<Product.Counter>
										<Product.Counter.Wrapper onClick={() => dispatch({ type: 'decrement', payload: { id: value?.id } })}>
											<Product.Counter.Decrement />
										</Product.Counter.Wrapper>
										<Product.Counter.Text>{value?.quantity}</Product.Counter.Text>
										<Product.Counter.Wrapper onClick={() => dispatch({ type: 'increment', payload: { id: value?.id } })}>
											<Product.Counter.Increment />
										</Product.Counter.Wrapper>
									</Product.Counter>
									<Product.Icon onClick={() => dispatch({ type: 'cancelcart', payload: { id: value?.id } })}>
										<Product.Trash onClick={() => error(value.title)} />
									</Product.Icon>
								</Product>
							);
						})
				...
				<Footer>
					...
					<Footer.Wrapper>
						...
						<Footer.TotalPrice>
							${state?.cart?.reduce((sum, product) => sum + product.price * product.quantity, 0)}.00
						</Footer.TotalPrice>
					</Footer.Wrapper>
					<Footer.Buttons>
						<Button height='40px'>Proceed To Checkout</Button>
						<Button height='40px' onClick={() => dispatch({ type: 'closeCart' })}>
							Continue Shopping
						</Button>
					</Footer.Buttons>
				</Footer>

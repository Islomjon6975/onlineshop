				...
				<Wrapper.Wrap>
					<Wrapper.Ul>
						{navbar.map(item => {
							return (
								!item.isPrivate && (
									<Wrapper.Li active={active(item.path)} key={item.id} to={item.path}>
										{item.title}
									</Wrapper.Li>
								)
							);
							...
							return (
								...
								!item.hidden && (
									<Wrapper.Li
										...
										onClick={!token ? () => dispatch({ type: 'openModal' }) : 'none'}
										key={item.id}
										to={token && item.path}>
										{item.title}
									</Wrapper.Li>
					...
					<Wrapper.Right>
						...
						{JSON.parse(localStorage.getItem('register')).role === 'seller' ? (
							...
							<Wrapper.ButtonWrapper>
								<Button
									onClick={() => {
										if (!token && localStorage.getItem('register')) {
